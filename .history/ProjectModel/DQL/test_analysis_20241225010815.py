...
# Statistiques de test
# Récompense moyenne (avg_reward): 3.5229

# Interprétation:
# La récompense moyenne mesure la performance globale de l'agent. Une valeur positive de 3.5229 indique que l'algorithme prend globalement des décisions qui améliorent les conditions du trafic. Plus cette valeur est élevée, mieux l'agent répond à l'objectif d'optimisation.
# Écart-type des récompenses (std_reward): 4.0702

# Interprétation:
# Cet écart-type montre la variabilité des récompenses. Une valeur de 4.0702 indique que les performances de l'agent fluctuent parfois de manière significative. Cela peut refléter des périodes où le trafic est bien régulé et d'autres où il l'est moins.
# Longueur moyenne des files d'attente (avg_queue): 1.7203

# Interprétation:
# Une longueur moyenne de file d'attente de 1.7203 véhicules par étape montre que l'algorithme parvient généralement à maintenir des files d'attente courtes, ce qui est un indicateur positif de régulation fluide.
# Vitesse moyenne (avg_speed): 5.9194 m/s

# Interprétation:
# Une vitesse moyenne de 5.9194 m/s indique que les véhicules circulent à une vitesse modérée. Cela reflète un trafic fluide, sans congestion majeure. Une vitesse plus élevée est généralement souhaitable mais doit être équilibrée avec la sécurité routière.
# Taux de congestion (congestion_rate): 0.0000

# Interprétation:
# Un taux de congestion nul (0.0000) signifie qu'il n'y a pas de périodes où le taux d'occupation de la route dépasse le seuil critique de 0.7. Cela montre que l'algorithme est très efficace pour prévenir les embouteillages.
# Taux de changement de phase (phase_change_rate): 0.1026

# Interprétation:
# Ce taux indique qu'en moyenne, 10.26% des étapes ont impliqué un changement de phase des feux de signalisation. Cela montre que l'algorithme ajuste les phases de manière dynamique mais pas excessive, ce qui est essentiel pour éviter des transitions trop fréquentes qui pourraient déstabiliser le flux de trafic.
